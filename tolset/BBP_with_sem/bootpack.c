...
void producer(struct BBParg *args) {
	...
	while (1) {
		...
		sem_wait(empty);
		sem_wait(mutex);
		cons_putstr0(task->cons, "acquired mutex lock\n");
		fifo32_put(fifo, cnt++);
		sprintf(str, "pproc gens %d\n", cnt - 1);
		cons_putstr0(task->cons, str);
		sem_signal(mutex);
		cons_putstr0(task->cons, "released mutex lock\n");
		sem_signal(full);
	}
}
...
void HariMain(void) {
    ...
    *((int *)0x0fe4) = (int)shtctl;
	...
	/* 从这里开始 */
    struct SHEET *sout, *ssout, *pout;
	sout = open_console(shtctl, memtotal); // 新建一个命令行窗口专门用于c2输出
	ssout = open_console(shtctl, memtotal); // 新建一个命令行窗口专门用于c1输出
	pout = open_console(shtctl, memtotal); // 新建一个命令行窗口专门用于p输出

	sheet_slide(sout, 512, 4);
	sheet_slide(ssout, 256, 4);
	sheet_slide(pout, 0, 4);
	sheet_updown(sout, shtctl->top);
	sheet_updown(ssout, shtctl->top);
	sheet_updown(pout, shtctl->top);
	struct BBParg agp, agc1, agc2;
	int buf[128];
	struct FIFO32 bbpfifo;
	fifo32_init(&bbpfifo, 128, buf, 0);		// 队列初始化
    struct TASK *tskp, *tskc1, *tskc2;
	int empty = 128, mutex = 1, full = 0;	// 信号量初始化
	int con_mutex = 1;						// 参数配置
	agp.empty = &empty;
    agp.full = &full;
    agp.mutex = &mutex;
    agp.fifo = &bbpfifo;
    agp.con_mutex = &con_mutex;				// 之前设置的控制台互斥锁，现在没有用处了（因为每个进程都有独立的输出窗口了）
    agc1 = agc2 = agp;
    agc1.id = 1;
    agc2.id = 2;
	agp.task = pout->task;
    agc1.task = ssout->task;				// 第一个消费者使用系统启动时建立的窗口
    agc2.task = sout->task;					// 第二个消费者使用我们单独建立的窗口
