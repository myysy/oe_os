...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, int memtotal)
{
	if (strcmp(cmdline, "mem") == 0 && cons->sht != 0) {
		...
		cmd_dir(cons);
	} else if (strncmp(cmdline, "alloc4k ", 8) == 0 && cons->sht != 0) {
		cmd_alloc4k(cons, cmdline);
...
void cmd_alloc4k(struct CONSOLE *cons, char *cmdline) {
	struct Buddy *buddy = (struct Buddy *) BUDDY_ADDR;
	int i, offset, tmp = 0;

	for (i = 8; cmdline[i] != 0; i++) {
		tmp = tmp * 10 + cmdline[i] - '0';
	}

	offset = Buddy_alloc_4k(buddy, tmp);

	char s[60];

	cons_putstr0(cons, "size(Byte) : ");
	sprintf(s, "%d\n", (tmp + 0xfff) & 0xfffff000);
	cons_putstr0(cons, s);

	cons_putstr0(cons, "size(4KB) : ");
	sprintf(s, "%d\n", ((tmp + 0xfff) & 0xfffff000) >> 0xc);
	cons_putstr0(cons, s);
