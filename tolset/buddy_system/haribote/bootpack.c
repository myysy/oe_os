...
void HariMain(void)
{
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	buf_back  = (unsigned char *) Buddy_alloc_4k(buddy, binfo->scrnx * binfo->scrny);
	...
	/* nihongo.fnt�̓ǂݍ��� */
	fat = (int *) Buddy_alloc_4k(buddy, 4 * 2880);
	...
	if (finfo != 0) {
		...
		nihongo = (unsigned char *) Buddy_alloc_4k(buddy, 16 * 256 + 32 * 94 * 47);
		for (i = 0; i < 16 * 256; i++) {
...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	unsigned char *buf = (unsigned char *) Buddy_alloc_4k(buddy, 512 * (165 + 16 * 10));
	sheet_setbuf(sht, buf, 512, 325, -1); /* �����F�Ȃ� */
	make_window8(buf, 512, 325, "console", 0);
	make_textbox8(sht, 8, 28, 240 + 256, 128 + 160, COL8_000000);
...
void close_constask(struct TASK *task)
{
	struct Buddy *buddy = (struct Buddy *) BUDDY_ADDR;
	task_sleep(task);
	Buddy_free(buddy, task->cons_stack);
	Buddy_free(buddy, (int) task->fifo.buf);
