    ...
    for (;;) {
        ...
        if (fifo32_status(&task->fifo) == 0) {
            ...
            if (i == 100) {
                ...
                putfonts8_asc_sht(sht_win_b, 40, 40, COL8_000000, COL8_C6C6C6,
                                  s, 11);
                count0 = count;
                timer_settime(timer_1s, 100);
            } else if (i == 4) {
            ...
            if (i == 3) { /* �J�[�\��OFF */
                if (cons.sht != 0) {
                    boxfill8(cons.sht->buf, cons.sht->bxsize, COL8_000000,
                             cons.cur_x, cons.cur_y, cons.cur_x + 7,
                             cons.cur_y + 15);
                }
                cons.cur_c = -1;
            }
            ...
            if (cons.sht != 0) {
                if (cons.cur_c >= 0) {
                    boxfill8(cons.sht->buf, cons.sht->bxsize, cons.cur_c,
                             cons.cur_x, cons.cur_y, cons.cur_x + 8,
                             cons.cur_y + 16);
                    // cons.cur_x, cons.cur_y, cons.cur_x + 7, cons.cur_y + 15);
                }
                // sheet_refresh(cons.sht, cons.cur_x, cons.cur_y, cons.cur_x +
                // 500, cons.cur_y + 500);
...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, int memtotal) {
    if (strcmp(cmdline, "mem") == 0 && cons->sht != 0) {
    ...
    } else if ((strncmp(cmdline, "attr +", 6) == 0 ||
                strncmp(cmdline, "attr -", 6) == 0) &&
               cmdline[7] == ' ') {
        ...
        struct MEMMAN *memman = (struct MEMMAN *)MEMMAN_ADDR;
        cmd_tmem_test(cons, memman, cmdline);
    } else if (cmdline[0] != 0) {
...
void cmd_mem_free(struct CONSOLE *cons, struct MEMMAN *man, char *cmdline) {
    ...
    char s[60];
    // sprintf(s, "addr: %d size: %d\n", addr, size);
    // cons_putstr0(cons, s);
    memman_free(man, addr, size);
    cmd_print_mem(cons, man);
}

void cmd_tmem_test(struct CONSOLE *cons, struct MEMMAN *man, char *cmdline) {
    char s[60];
#if TYPE == 1
    sprintf(s, "test the first algorithem\n");
#elif TYPE == 2
    sprintf(s, "test the best algorithem\n");
#elif TYPE == 3
    sprintf(s, "test the worst algorithem\n");
#endif
    cons_putstr0(cons, s);
    cmd_print_mem(cons, man);
    sprintf(s, "malloc 283136\n");
    cons_putstr0(cons, s);
    cmd_mem_alloc(cons, man, s);
    sprintf(s, "malloc 161792\n");
    cons_putstr0(cons, s);
    cmd_mem_alloc(cons, man, s);
    sprintf(s, "mfree 4096 283136\n");
    cons_putstr0(cons, s);
    cmd_mem_free(cons, man, s);
    sprintf(s, "malloc 202240\n");
    cons_putstr0(cons, s);
    cmd_mem_alloc(cons, man, s);
    sprintf(s, "malloc 40448\n");
    cons_putstr0(cons, s);
    cmd_mem_alloc(cons, man, s);
    sprintf(s, "mfree 287232 161792\n");
    cons_putstr0(cons, s);
    cmd_mem_free(cons, man, s);
}
    ...
    for (i = 0; i < max_index; i++) {
        ...
        if (finfo[i].name[0] != 0xe5) {
            ...
            if ((finfo[i].type & 0x18) == 0) {
                //系统文件或隐藏文件不显示
                // if (finfo[i].type & 0x04) == 0 && (finfo[i].type & 0x02) ==
                // 0){
...
void cmd_mkdir(struct CONSOLE *cons, int *fat, char *cmdline) {
    ...
    struct FILEINFO *new_dir =
        (struct FILEINFO *)Buddy_alloc_4k(memman, sizeof(struct FILEINFO));
    int i, j;
    char s[30];
    char name[12] = "";
...
void cmd_touch(struct CONSOLE *cons, int *fat, char *cmdline) {
    ...
    for (i = 6, j = 0; cmdline[i] != '.' && cmdline[i] != 0 && j < 8;
         i++, j++) {
...
void cmd_vi(struct CONSOLE *cons, int *fat, char *cmdline) {
    ...
    if (fh == 0) {  //不存在
        ...
        for (i = 3, j = 0; cmdline[i] != '.' && cmdline[i] != 0 && j < 8;
             i++, j++) {
...
void cmd_del(struct CONSOLE *cons, int *fat, char *cmdline) {
    ...
    //存在
    if (is_exsit != 0 && is_exsit->type != 0 &&
        ((is_exsit->type & 0x18) == 0)) {
...
void cmd_cdparent() {
    ...
    current_dir = (struct FILEINFO *)(ADR_DISKIMG + 0x004200 +
                                      (current_clustno - 2) * 512);
}
...
void cmd_tree(struct CONSOLE *cons, int clustno, int p) {
    ...
    struct FILEINFO *finfo =
        (struct FILEINFO *)(ADR_DISKIMG + 0x004200 + (clustno - 2) * 512);
...
char *cmd_path(struct CONSOLE *cons, struct FILEINFO finfo, char *s) {
    ...
    finfo = ((struct FILEINFO *)(ADR_DISKIMG + 0x004200 +
                                 (clustno - 2) * 512))[pos];
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline) {
    ...
    if (finfo != 0) {
        ...
        if (appsiz >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
            ...
            task->ds_base = (int)q;
            set_segmdesc(task->ldt + 0, appsiz - 1, (int)p,
                         AR_CODE32_ER + 0x60);
            set_segmdesc(task->ldt + 1, segsiz - 1, (int)q,
                         AR_DATA32_RW + 0x60);
            for (i = 0; i < datsiz; i++) {
                q[esp + i] = p[dathrb + i];
            }
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx,
    ...
    if (edx == 1) {
        ...
        sheet_updown(
            sht,
            shtctl->top); /* ���̃}�E�X�Ɠ��������ɂȂ��悤�Ɏw���F �}�E�X�͂��̏��ɂȂ� */
        reg[7] = (int)sht;
    } else if (edx == 6) {
