...
void dining(struct DINE *args){//by xhf 
	int *chopstick = args->chopstick;
	int i = args->i;
	struct FIFO32 *fifo = args->fifo;
    struct TASK *task = args->task;
    char str[128];
    while(1) {
    	while(task->cons == 0)avoid_sleep();;
    	if(i%2){
    		sem_wait(chopstick[(i+1)%5]);
			sprintf(str, "No.%d: I picked up chopstick %d\n", i, (i+1)%5);
			cons_putstr0(task->cons, str);
