...
void HariMain(void)
{
	...
	*((int *) 0x0fe8) = (int) nihongo;
	...
	// sprintf(sss, "%hu\n", getldt());
	for (;;) {
		// if (key_win->task->cons != 0) {
		// 	cons_putstr0(key_win->task->cons, sss);
...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	unsigned char *buf = (unsigned char *) memman_alloc_4k(memman, 512 * 325);
	sheet_setbuf(sht, buf, 512, 325, -1); /* ?????F??? */
	make_window8(buf, 512, 325, "console", 0);
	make_textbox8(sht, 8, 28, 496, 288, COL8_000000);
...
void close_console(struct SHEET *sht)
{
	...
	memman_free_4k(memman, (int) sht->buf, 512 * 325);
	sheet_free(sht);
	close_constask(task);
	return;
}
